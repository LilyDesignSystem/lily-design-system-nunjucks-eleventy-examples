...
// ported from the canonical SvelteKit reference
// (lily-design-system-svelte-sveltekit-examples/e2e/book-an-appointment.spec.ts)
// to this app's static-site route shape and vanilla progressive-enhancement
// JS. Full write-up: docs/patterns/book-an-appointment.md.
//
...
// base.njk injects the managed theme <link> from a pre-paint <script>, so
// the stylesheet finishes loading asynchronously, after `page.goto()`
// resolves. Scanning (or measuring) before it applies is a false failure
// unrelated to page content -- wait for the managed link's sheet to have
// rules before running axe. See e2e/accessibility.spec.ts for the full
// rationale.
...
test.describe('book an appointment: per-step validation', () => {
  ...
  test('reason step with the conditional textarea open is axe-clean', async ({ page }) => {
    await page.goto('/book-an-appointment/');
    await page.getByRole('button', { name: 'Start now' }).click();
    await page.getByLabel('Something else').check();
    await expect(page.locator('#reason-other-detail')).toBeVisible();
    await expectNoViolations(page, 'step 1: reason ("something else" textarea open)');
  });
